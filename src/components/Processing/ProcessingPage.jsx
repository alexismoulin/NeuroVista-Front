...
export default function ProcessingPage({ setPage }) {
    const steps = [
        {
            key: "dicom",
            completedText: "Dicom upload Completed",
            loadingText: "Dicom upload in Progress",
        },
        {
            key: "nifti",
            completedText: "Nifti creation completed",
            loadingText: "Nifti creation in Progress",
        },
        {
            key: "recon",
            completedText: "Brain reconstruction completed",
            loadingText: "Brain reconstruction in Progress (very long - can take up to 16h)",
        },
        {
            key: "lesions",
            completedText: "Potential lesions analysis completed",
            loadingText: "Potential lesions analysis in Progress",
        },
        {
            key: "subs",
            completedText: "Subcortical segmentations completed",
            loadingText: "Subcortical segmentations in Progress",
        },
        {
            key: "hyp",
            completedText: "Hypothalamus segmentations completed",
            loadingText: "Hypothalamus segmentations in Progress",
        },
        {
            key: "json",
            completedText: "Json files created",
    ...
    // Debug: log the keys currently in the response object
    console.log(Object.keys(response));

    const completedCount = steps.reduce(
        (count, { key }) => count + (response[key] ? 1 : 0),
        0
    );
            ...
            <section className="bg-white w-11/12">
                ...
                <ol className="list-decimal text-slatey font-merriweather border-b-2 my-4">
                    {steps.map(({ key, completedText, loadingText }) => {
                        ...
                        // If the pipeline has finished and this step was not marked complete, mark it as failed.
                        if (!loading) {
                            const failedStep = failedSteps.find((f) => f.key === `failed_${key}`);
                            return (
                                <li key={key} className="my-4 mx-8">
                                    <FailedStep
                                        stepText={failedStep ? failedStep.completedText : "Step failed"}
                                    />
                                </li>
                            );
                        }
                ...
                <div className="flex justify-center p-6">
                    <PrimaryButton
                        ...
                        // Enable the button if either the json step has completed or failed.
